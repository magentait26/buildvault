                    ...
                    <div className="absolute top-9 left-0 right-0 z-20 bg-white border border-slate-200 rounded-lg shadow-xl max-h-64 overflow-y-auto text-xs p-1 animate-in fade-in slide-in-from-top-1">
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 hover:bg-slate-50 flex items-center justify-between rounded-md ${
                          selectedProjectId === 'all' ? 'bg-slate-50 font-semibold text-slate-900 border-l-2 border-slate-900' : 'text-slate-650'
                        }`}
                      >
                      ...
                      {projects.map(p => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 hover:bg-slate-50 flex items-center justify-between rounded-md ${
                              selectedProjectId === p.id ? 'bg-slate-50 font-semibold text-slate-900 border-l-2 border-slate-900' : 'text-slate-650'
                            }`}
                          >
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-3.5 px-4 vertical-align-top">
                            {proj ? (
                              <div className="space-y-0.5">
                                <span className="text-xs font-semibold text-slate-700 block">{proj.name}</span>
                          ...
                          <td className="py-3.5 px-4">
                            <div className="space-y-1.5 max-w-[200px]">
                              ...
                              <div className="flex flex-wrap gap-1">
                                {doc.tags.map(t => (
                                  <span key={t} className="text-[9px] bg-slate-50 border border-slate-150 text-slate-500 px-1 py-0.2 rounded font-sans">
                                    #{t}
                                  </span>
                                ))}
                              </div>
                          ...
                          <td className="py-3.5 px-4">
                            <span className={`inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wider px-2.5 py-0.5 rounded-full ${
                              ...
                              doc.status === 'Archived' ? 'bg-slate-100 text-slate-700' :
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-between items-start pb-4 border-b border-slate-100">
                ...
                <button 
                  ...
                  className="p-1.5 hover:bg-slate-100 text-slate-400 hover:text-slate-800 rounded-lg border border-slate-150 cursor-pointer bg-white"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-xs font-sans">
                ...
                <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                  ...
                  <span className="font-semibold text-slate-750">{previewDocProject?.name || 'Global'}</span>
                </div>
              </div>
                  ...
                  {previewDocVersions.map((v, idx) => (
                    <div key={v.id} className="p-3 bg-white text-xs flex justify-between items-start hover:bg-slate-50/20">
                      <div className="space-y-1">
                        ...
                        <p className="text-[11px] text-slate-500 italic">"{v.comment || 'Checkin state modification file content'}"</p>
              ...
              {isApprovalsModuleOn && previewDoc.status !== 'Active' && !isDocUnderActiveApproval && (
                <div className="space-y-2 bg-slate-50 p-4 rounded-xl border border-slate-200">
                  <h4 className="text-xs font-semibold text-slate-800">Dispatch Statutory Validation Workflow</h4>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white border border-slate-200 rounded-xl shadow-2xl max-w-md w-full p-6 animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleUploadSubmit} className="space-y-4 text-xs font-sans">
              ...
              {validationError && (
                <div className="bg-rose-50 border border-rose-200 text-rose-700 p-3 rounded-lg flex items-start gap-2.5">
                ...
                {attachedFile ? (
                  <div className="flex items-center justify-between bg-white border border-slate-200 rounded-md p-2.5 shadow-3xs">
                    ...
                    <button
                      ...
                      className="text-xxs text-rose-600 hover:text-rose-800 font-bold ml-2 underline cursor-pointer"
                    >
                      Clear
                    </button>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-slate-600 block">Filing Status</label>
                  <select
                    value={uploadStatus}
              ...
              <div className="space-y-1">
                <label className="font-semibold text-slate-600 block">Tags (comma split)</label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                <label className="font-semibold text-slate-600 block">Brief Description</label>
                <textarea
              ...
              <div className="space-y-1">
                <label className="font-semibold text-slate-600 block">Initial Upload Comment</label>
                <input
                  type="text"
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4 text-xs font-sans">
              ...
              <div className="pt-2 flex gap-2">
                <button
                  ...
                  className="flex-1 py-2 border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-lg font-semibold text-center transition-all cursor-pointer text-xs"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg font-semibold text-center transition-all cursor-pointer text-xs"
                >
          ...
          <div className="relative bg-white border border-slate-200 rounded-xl shadow-2xl max-w-sm w-full p-6 animate-in zoom-in-95 duration-200 z-10">
            ...
            <div className="pt-4 flex gap-2">
              <button
                ...
                disabled={isActionLoading}
                className="flex-1 py-1.5 border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-lg text-xs font-semibold transition-all cursor-pointer text-center"
              >
                No, Cancel
              </button>
              <button
                ...
                disabled={isActionLoading}
                className={`flex-1 py-1.5 text-white rounded-lg text-xs font-semibold transition-all cursor-pointer text-center ${
                  actionConfirm.type === 'delete' 
